...
| Authentication | One evidence-selected first-party real-browser return, otherwise one honest native direct path, otherwise explicit unsupported | This implements D-01 without a method selector, fallback, browser-state extraction, or access-control workaround. |
...
- [ ] Native scene — one SwiftUI authentication/compatibility scene consumes typed client state; web routing is not applicable.
- [ ] Secure persistence — no database is introduced; Keychain add/read/update/delete is exercised through the app-owned adapter.
- [ ] UI interaction — explicit Retry and official-site actions are wired to the typed client/presentation boundary without live automatic requests.
