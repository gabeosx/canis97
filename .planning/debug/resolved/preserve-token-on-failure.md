...
next_action: Complete. Offline regression, mutation/revert, full-suite, and static-authority verification prove this persistence-boundary defect is resolved. Live provider verification was intentionally out of scope and is not required to close this debug record.
bug_class: bohrbug
...
verification:
  ...
  live_provider_verification: { result: intentionally_not_run, reason: "This defect is fully exercised by injected Keychain storage and offline authentication composition. The user explicitly excluded live provider requests and login from the fix scope." }
files_changed:
